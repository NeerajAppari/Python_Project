...
##                withaa=Enter(wintdr)
##                withaa.place(x=200,y=500)
##            
                osde5=osd5+f'\\{last5}{first5}'
                with open (f"{osde5}acc.txt",'r') as bal1:
                    d=str(bal1.read())
                    ...
                    if str(withaa)<=d:
                       ...
                       transo()
                with open(f"{osde2}trans.txt",'a') as tbal1:
                    tbal1.writelines(f'\n {str(final_ammount)}')
...
##                    withaa.place(x=200,500)
        ...
        def deposit():
            wintdepo=Tk()
            wintdepo.title("deposit")
            wintdepo.geometry("2000x1000")
            side=Label(wintdepo,width=14,height=200)
            side.configure(bg="slate blue")
            side.place(x=0,y=0)
            side1=Label(wintdepo,width=14,height=200)
            side1.configure(bg="slate blue")
            side1.place(x=1440,y=0)
            side2=Label(wintdepo,width=1,height=200)
            side2.configure(bg="black")
            side2.place(x=1438,y=0)
            side3=Label(wintdepo,width=1,height=200)
            side3.configure(bg="black")
            side3.place(x=95,y=0)
            ...
            tb3=Button(wintdepo,text="Back",width=12,height=2,bg="navy",fg="white",command=gtt3)
        ...
        def balance():
            ...
            if os.path.exists(osd3):
                ...
                with open (f"{osde3}acc.txt","r") as d:
                   
                    displabel3=Label(wintbal,font=('Arial Black',20,'bold'),text=d.read())
                    displabel3.pack()
